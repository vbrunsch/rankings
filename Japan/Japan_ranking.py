...
focus = df.drop(['patientId', 'ageBracket', 'gender', 'patientStatus', 'mhlwPatientNumber', 'prefecturePatientNumber', 'confirmedPatient', 'residence', 'relatedPatients', 'knownCluster', 'cityPrefectureNumber', 'deceasedDate', 'deceasedReportedDate', 'deathSourceURL'], axis = 1)#.set_index('dateAnnounced')
focus['new'] = 1
pref_only = focus.groupby('detectedPrefecture').sum().T
pref_only = pref_only.drop(['Unspecified','Port Quarantine'], axis = 1)
pref= focus.groupby(['dateAnnounced','detectedPrefecture']).sum()
pref = pref.reset_index(level=1)
pref.index = pd.to_datetime(pref.index, dayfirst=True)
tim = pref.index.max()
idx = pd.date_range('01-24-2020', tim)


cols=['Prefectures','COVID-Free Days','New Cases in Last 14 Days', 'Last7', 'Previous7']
collect = []

for d in pref_only.columns:
    cas = pref[pref['detectedPrefecture']==d].drop(['detectedPrefecture'], axis = 1)
...
tab.columns = ['Prefectures', 'COVID-Free Days', 'New Cases in Last 14 Days', 'Last 7 Days', 'Pct Change']
...
tab = tab[['Rank', 'Prefectures', 'COVID-Free Days', 'New Cases in Last 14 Days','Last 7 Days','Percent Change']]       
...
try:        
    with open(f'Japan_Prefectures.html', 'w', encoding="utf-8") as out:
