...
que = f'https://gis.hochsauerlandkreis.de/arcgis6/rest/services/COVID19/MapServer/2/query?where=0%3D0&outFields=%2A&f=json'
...
gem = re.findall('SDEatHSK.BASIS.COVID19_GEMEINDEN.GEMEINDENAME":"(.*?)"',t3)
inf = re.findall('SDEatHSK.BASIS.COVID19_aktuell.Bestaetigte_Faelle":(.*?),',t3)
...
df = df.astype(int)
print(df)
df.to_csv(f'Germany/NRW/Hochsauerlandkreis/data/Hochsauerlandkreis_{tod}.csv')
...
old7 = pd.read_csv(f'Germany/NRW/Hochsauerlandkreis/data/Hochsauerlandkreis_{da7s}.csv', index_col = 0)
old14 = pd.read_csv(f'Germany/NRW/Hochsauerlandkreis/data/Hochsauerlandkreis_{da14s}.csv', index_col = 0)
...
zus = zus.replace('Eslohe','Eslohe (Sauerland)')
zus = zus.replace('Sundern','Sundern (Sauerland)')
zus = zus.set_index('Gemeinde')
zus.index.name = None
zus['Gemeinde'] = zus.index

print(zus)
zus.to_csv(f'Germany/NRW/Hochsauerlandkreis/data/Hochsauerlandkreis_for_dw14_7.csv') 
...
try:        
    with open(f'Hochsauerlandkreis.html', 'w', encoding="utf-8") as out:
