...
url_s = 'https://corona.kreis-borken.de/api/data'
...
inf = re.findall('INFIZIERTE_SUM":(.*?),',t)
infy = re.findall('INFIZIERTE_SUM_VORTAG":(.*?),',t)

df = pd.DataFrame(data = inf, index = gem)
df.columns = [tod]
df[yes] = infy
print(df)
df.to_csv(f'Germany/NRW/Borken/data/Borken_{tod}.csv')
...
old7 = pd.read_csv(f'Germany/NRW/Borken/data/Borken_{da7s}.csv', index_col = 0)
old14 = pd.read_csv(f'Germany/NRW/Borken/data/Borken_{da14s}.csv', index_col = 0)
...
zus = zus.replace('Gronau','Gronau (Westf.)')
zus = zus.set_index('Gemeinde')
zus.index.name = None
zus['Gemeinde'] = zus.index

print(zus)
zus.to_csv(f'Germany/NRW/Borken/data/Borken_for_dw14_7.csv') 
...
try:        
    with open(f'Borken.html', 'w', encoding="utf-8") as out:
