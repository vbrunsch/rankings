...
pdf = re.findall('target="_blank" href="(.*?)" class="csslink_PDF">hier', t)
#soup = BeautifulSoup(t, "lxml")
#container = soup.find("href",attrs={'class': 'csslink_PDF'})
print(pdf)

pdf_path = 'https://www.kreis-olpe.de' + pdf[0]
...
old = pd.read_csv(f'Germany/NRW/Olpe/data/Olpe_{da7s}.csv', index_col = 0)

zus = df.copy()
zus['last7'] = zus[zus.columns[1]].astype(int) - old[old.columns[1]].astype(int) 
...
zus.to_csv(f'Germany/NRW/Olpe/data/Olpe_for_dw14_7.csv')#Germany/NRW/MK/data/ 
print(zus)
...
try:        
    with open(f'Olpe.html', 'w', encoding="utf-8") as out:
