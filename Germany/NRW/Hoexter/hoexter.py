import pandas as pd
dfs = pd.read_html('https://webservice.amstools.de/covid/getdatadisplay2.php?sender=radioho&element=hoexter')
df = dfs[0]
df = df.set_index('Ort')
df = df[['Gesamtzahl']]
...
old7 = pd.read_csv(f'Germany/NRW/Hoexter/data/Höxter_{da7s}.csv', index_col = 0)
old14 = pd.read_csv(f'Germany/NRW/Hoexter/data/Höxter_{da14s}.csv', index_col = 0)
...
zus = zus.replace('HÃ¶xter','Höxter')
zus = zus.replace('MarienmÃ¼nster','Marienmünster')
zus = zus.set_index('Gemeinde')
zus.index.name = None
zus['Gemeinde'] = zus.index

print(zus)
zus.to_csv(f'Germany/NRW/Hoexter/data/Höxter_for_dw14_7.csv') 
...
try:        
    with open(f'Hoexter.html', 'w', encoding="utf-8") as out:
