...
que = 'https://services-eu1.arcgis.com/H1pvqvVi8lTTXlkG/arcgis/rest/services/Fallzahlen_Gemeinden_Join/FeatureServer/0/query?where=0%3D0&outFields=%2A&f=json'
...
inf = re.findall('Infizierte":(.*?),',t3)
infy = re.findall('Neu_Infizi":(.*?),',t3)

df = pd.DataFrame(data = inf, index = gem)
df.columns = [tod]
df['neu'] = infy
print(df)
df.to_csv(f'Germany/NRW/Lippe/data/Lippe_{tod}.csv')
...
old7 = pd.read_csv(f'Germany/NRW/Lippe/data/Lippe_{da7s}.csv', index_col = 0)
old14 = pd.read_csv(f'Germany/NRW/Lippe/data/Lippe_{da14s}.csv', index_col = 0)
...
print(zus)
zus.to_csv(f'Germany/NRW/Lippe/data/Lippe_for_dw14_7.csv') 
...
try:        
    with open(f'Lippe.html', 'w', encoding="utf-8") as out:
