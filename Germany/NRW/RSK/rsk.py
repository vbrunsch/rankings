...
que = 'https://services3.arcgis.com/83sMx9VznDQWxJwO/arcgis/rest/services/CoronaFaelle2020/FeatureServer/0/query?where=0%3D0&outFields=%2A&f=json'
...
inf = re.findall('Infektionen":(.*?),',t3)

df = pd.DataFrame(data = inf, index = gem)
df.columns = [tod]

print(df)
df.to_csv(f'Germany/NRW/RSK/data/Rhein-Sieg-Kreis_{tod}.csv')
...
old7 = pd.read_csv(f'Germany/NRW/RSK/data/Rhein-Sieg-Kreis_{da7s}.csv', index_col = 0)
old14 = pd.read_csv(f'Germany/NRW/RSK/data/Rhein-Sieg-Kreis_{da14s}.csv', index_col = 0)
...
zus = zus.replace('Hennef','Hennef (Sieg)')
zus = zus.set_index('Gemeinde')
zus.index.name = None
zus['Gemeinde'] = zus.index

sk = pd.read_csv('https://raw.githubusercontent.com/vbrunsch/rankings/main/Germany/NRW/SKs/data/SKs_for_dw14_7.csv', index_col=0)
zus = zus.append(sk.loc[['Bonn']])
print(zus)
zus.to_csv(f'Germany/NRW/RSK/data/Rhein-Sieg-Kreis_for_dw14_7.csv') 
...
try:        
    with open(f'Rhein-Sieg-Kreis.html', 'w', encoding="utf-8") as out:
