import pandas as pd
dfs = pd.read_html('https://webservice.amstools.de/covid/getdatadisplay2.php?sender=radiowa')
df = dfs[0]
df = df.set_index('Ort')
df = df[['Gesamtzahl']]
...
print(df)
df.to_csv(f'Germany/NRW/Warendorf/data/Warendorf_{tod}.csv')
...
old7 = pd.read_csv(f'Germany/NRW/Warendorf/data/Warendorf_{da7s}.csv', index_col = 0)
old14 = pd.read_csv(f'Germany/NRW/Warendorf/data/Warendorf_{da14s}.csv', index_col = 0)
...
print(zus)
zus.to_csv(f'Germany/NRW/Warendorf/data/Warendorf_for_dw14_7.csv') 
...
try:        
    with open(f'Warendorf.html', 'w', encoding="utf-8") as out:
