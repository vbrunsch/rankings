...
dfs = pd.read_html('https://www.kreis-unna.de/nachrichten/n/update-coronavirus-1/')
...
df = df[4:]
df.columns = ['Gemeinde',df.columns[1],df.columns[2],df.columns[3]]
...
df[df.columns[2]] = df[df.columns[2]].str.replace('+','')
df = df.astype(int)
df.to_csv(f'Germany/NRW/Unna/data/Unna_{tod}.csv')
print(df)
...
old7 = pd.read_csv(f'Germany/NRW/Unna/data/Unna_{da7s}.csv', index_col = 0)
old14 = pd.read_csv(f'Germany/NRW/Unna/data/Unna_{da14s}.csv', index_col = 0)

zus = df.copy()
zus['last7'] = zus[zus.columns[1]].astype(int) - old7[old7.columns[1]].astype(int)
zus['last14'] = zus[zus.columns[1]].astype(int) - old14[old14.columns[1]].astype(int)
...
try:        
    with open(f'Unna.html', 'w', encoding="utf-8") as out:
