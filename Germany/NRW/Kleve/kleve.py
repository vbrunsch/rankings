...
url_s = 'https://www.goch.de/de/aktuelles/corona/'
t = requests.get(url_s).text
bed = re.findall('([^<p>]+?) in Bedburg-Hau', t)
emm = re.findall('([^<p>]+?) in Emmerich', t)
gel = re.findall('([^<p>]+?) in Geldern', t)
goc = re.findall('([^<p>]+?) in Goch', t)
iss = re.findall('([^<p>]+?) in Issum', t)
kal = re.findall('([^<p>]+?) in Kalkar', t)
ker = re.findall('([^<p>]+?) in Kerken', t)
kev = re.findall('([^<p>]+?) in Kevelaer', t)
kle = re.findall('([^<p>]+?) in Kleve', t)
kra = re.findall('([^<p>]+?) in Kranenburg', t)
ree = re.findall('([^<p>]+?) in Rees', t)
rhe = re.findall('([^<p>]+?) in Rheurdt', t)
stra = re.findall('([^<p>]+?) in Straelen', t)
ued = re.findall('([^<p>]+?) in Uedem', t)
wac = re.findall('([^<p>]+?) in Wachtendonk', t)
wee = re.findall('([^<p>]+?) in Weeze', t)

gem = ['Bedburg-Hau','Emmerich','Geldern','Goch','Issum','Kalkar','Kerken','Kevelaer','Kleve','Kranenburg','Rees','Rheurdt','Straelen','Uedem','Wachtendonk','Weeze']
df = pd.DataFrame(data = [bed[0],emm[0],gel[0],goc[0],iss[0],kal[0],ker[0],kev[0],kle[0],kra[0],ree[0],rhe[0],stra[0],ued[0],wac[0],wee[0]], index = gem, columns=['Gesamtfallzahlen'])
...
old7 = pd.read_csv(f'Germany/NRW/Kleve/data/Kleve_{da7s}.csv', index_col = 0)
old14 = pd.read_csv(f'Germany/NRW/Kleve/data/Kleve_{da14s}.csv', index_col = 0)
...
zus = zus.replace('Emmerich','Emmerich am Rhein')
zus = zus.set_index('Gemeinde')
zus.index.name = None
zus['Gemeinde'] = zus.index

print(zus)
zus.to_csv(f'Germany/NRW/Kleve/data/Kleve_for_dw14_7.csv') 
...
try:        
    with open(f'Kleve.html', 'w', encoding="utf-8") as out:
