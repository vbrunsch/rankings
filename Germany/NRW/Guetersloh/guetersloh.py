...
que = f'https://webgis.kreis-guetersloh.de/wss/service/ags-relay/agskreisgt/guest/arcgis/rest/services/gesundheit/CoronaFallzahlen_Inzidenzen/MapServer/1/query?where=tag_id+%3E+{tag}&text=&objectIds=&time=&geometry=&geometryType=esriGeometryEnvelope&inSR=&spatialRel=esriSpatialRelIntersects&relationParam=&outFields=datum%2Ctag_id%2Cort%2Csumme_bestaetigt&returnGeometry=false&returnTrueCurves=false&maxAllowableOffset=&geometryPrecision=&outSR=&returnIdsOnly=false&returnCountOnly=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&returnZ=false&returnM=false&gdbVersion=&historicMoment=&returnDistinctValues=false&resultOffset=&resultOffset=&queryByDistance=&returnExtentsOnly=false&datumTransformation=&parameterValues=&rangeValues=&quantizationParameters=&f=pjson'
...
inf = re.findall('summe_bestaetigt": (.*)',t3)
infy = re.findall('datum": "(.*?)",',t3)
...
df = df[1:]
print(df)
df.to_csv(f'Germany/NRW/Guetersloh/data/Guetersloh_{tod}.csv')
...
for i in hdf.index.unique():
  idf = df.loc[[i]].copy()
  idf['neu'] = idf['"summe_bestaetigt"'].astype(int).diff()
...
print(zus)
zus.to_csv(f'Germany/NRW/Guetersloh/data/Guetersloh_for_dw14_7.csv') 
...
try:        
    with open(f'Guetersloh.html', 'w', encoding="utf-8") as out:
