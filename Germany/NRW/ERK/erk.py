...
url_s = 'https://static.dwcdn.net/data/fOl9I.csv'
urlD = requests.get(url_s).content
df = pd.read_csv(io.StringIO(urlD.decode('utf-8')))
df = df.set_index('Wohnort')
df = df[[' Infizierte Gesamt']]
...
old7 = pd.read_csv(f'Germany/NRW/ERK/data/ERK_{da7s}.csv', index_col = 0)
old14 = pd.read_csv(f'Germany/NRW/ERK/data/ERK_{da14s}.csv', index_col = 0)
...
zus = zus.replace('Wetter','Wetter (Ruhr)')
zus = zus.set_index('Gemeinde')
zus.index.name = None
zus['Gemeinde'] = zus.index

print(zus)
zus.to_csv(f'Germany/NRW/ERK/data/ERK_for_dw14_7.csv') 
...
try:        
    with open(f'Ennepe-Ruhr-Kreis.html', 'w', encoding="utf-8") as out:
