...
que = 'https://services.arcgis.com/0EDx2Zp3hlT2xyF5/arcgis/rest/services/Gemeinden_Data_Corona_Sicht/FeatureServer/0/query?where=0%3D0&outFields=%2A&f=json'
...
gem = re.findall('GEMNAME":"(.*?)"',t3)
inf = re.findall('infiziert":(.*?),',t3)

df = pd.DataFrame(data = inf, index = gem)
df.columns = [tod]
print(df)
df.to_csv(f'Germany/NRW/Coesfeld/data/Coesfeld_{tod}.csv')
...
old7 = pd.read_csv(f'Germany/NRW/Coesfeld/data/Coesfeld_{da7s}.csv', index_col = 0)
old14 = pd.read_csv(f'Germany/NRW/Coesfeld/data/Coesfeld_{da14s}.csv', index_col = 0)
...
print(zus)
zus.to_csv(f'Germany/NRW/Coesfeld/data/Coesfeld_for_dw14_7.csv') 
...
try:        
    with open(f'Coesfeld.html', 'w', encoding="utf-8") as out:
