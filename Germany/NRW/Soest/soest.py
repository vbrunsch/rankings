...
que = 'https://gis.kreis-soest.de/server/rest/services/Krisenstab/Corona_Dashboard_Kreis_Soest_Oeffentlich/MapServer/0/query?where=0%3D0&outFields=%2A&f=json'
...
df = df.astype(int)
print(df)
df.to_csv(f'Germany/NRW/Soest/data/Soest_{tod}.csv')
...
old7 = pd.read_csv(f'Germany/NRW/Soest/data/Soest_{da7s}.csv', index_col = 0)
old14 = pd.read_csv(f'Germany/NRW/Soest/data/Soest_{da14s}.csv', index_col = 0)
...
zus = zus.replace('Wickede','Wickede (Ruhr)')
zus = zus.set_index('Gemeinde')
zus.index.name = None
zus['Gemeinde'] = zus.index

print(zus)
zus.to_csv(f'Germany/NRW/Soest/data/Soest_for_dw14_7.csv') 
...
try:        
    with open(f'Soest.html', 'w', encoding="utf-8") as out:
