...
que = 'https://services.arcgis.com/r7GhuZjF2UKLvKNF/arcgis/rest/services/CoronavirusSituation_CoronavirusSituation_View/FeatureServer/0/query?where=0%3D0&outFields=%2A&f=json'
...
gem = re.findall('City":"(.*?)"',t3)
inf = re.findall('confirmed":(.*?),',t3)

df = pd.DataFrame(data = inf, index = gem)
df.columns = [tod]
df.index = df.index.str.replace('GL','Bergisch Gladbach')
df.index = df.index.str.replace('BU','Burscheid')
df.index = df.index.str.replace('KU','Kürten')
df.index = df.index.str.replace('LE','Leichlingen')
df.index = df.index.str.replace('OD','Odenthal')
df.index = df.index.str.replace('OV','Overath')
df.index = df.index.str.replace('RO','Rösrath')
df.index = df.index.str.replace('WK','Wermelskirchen')

print(df)
df.to_csv(f'Germany/NRW/RBK/data/Rheinisch-Bergischer-Kreis_{tod}.csv')
...
old7 = pd.read_csv(f'Germany/NRW/RBK/data/Rheinisch-Bergischer-Kreis_{da7s}.csv', index_col = 0)
old14 = pd.read_csv(f'Germany/NRW/RBK/data/Rheinisch-Bergischer-Kreis_{da14s}.csv', index_col = 0)
...
zus = zus.replace('Leichlingen','Leichlingen (Rhld.)')
zus = zus.set_index('Gemeinde')
zus.index.name = None
zus['Gemeinde'] = zus.index

print(zus)
zus.to_csv(f'Germany/NRW/RBK/data/Rheinisch-Bergischer-Kreis_for_dw14_7.csv') 
...
try:        
    with open(f'Rheinisch-Bergischer-Kreis.html', 'w', encoding="utf-8") as out:
