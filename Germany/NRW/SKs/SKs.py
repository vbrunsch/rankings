import pandas as pd
dfs = pd.read_html('https://vbrunsch.github.io/rankings/Germany.html')
dfs = dfs[0]
df = dfs[dfs['District/County Town'].str.contains(', SK')]
...
import numpy as np
zus = pd.DataFrame()
zus['last7'] = df['Last 7 Days']
zus['last14'] = df['New Cases in Last 14 Days']
...
zus['Gemeinde'] = df['District/County Town']
#ags = pd.read_csv('AGS_SKs.csv', index_col= 0, dtype=str)
zus = zus.set_index('Gemeinde')
zus.index.name = None
#zus['AGS'] = ags['AGS']
zus['Gemeinde'] = zus.index
#zus = zus.set_index('AGS')
zus.to_csv(f'Germany/NRW/SKs/data/SKs_for_dw14_7.csv')
print(zus)
...
try:        
    with open(f'SKs.html', 'w', encoding="utf-8") as out:
