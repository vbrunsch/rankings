...
que = 'https://services-eu1.arcgis.com/B4AGMbvhuqY0JRyu/arcgis/rest/services/CoronaFallzahlenGemeinden/FeatureServer/0/query?where=0%3D0&outFields=%2A&f=json'
...
inf = re.findall('Summe":(.*?),',t3)

df = pd.DataFrame(data = inf, index = gem)
df.columns = [tod]
print(df)
df.to_csv(f'Germany/NRW/Dueren/data/Dueren_{tod}.csv')
...
old7 = pd.read_csv(f'Germany/NRW/Dueren/data/Dueren_{da7s}.csv', index_col = 0)
old14 = pd.read_csv(f'Germany/NRW/Dueren/data/Dueren_{da14s}.csv', index_col = 0)
...
print(zus)
zus.to_csv(f'Germany/NRW/Dueren/data/Dueren_for_dw14_7.csv') 
...
try:        
    with open(f'Dueren.html', 'w', encoding="utf-8") as out:
