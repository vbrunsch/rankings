...
inf = re.findall('IndexfaelleInsgesamt":(.*?),',t3)
infn = re.findall('VeraenderungVortagIi":(.*?),',t3)

df = pd.DataFrame(data = inf, index = gem)
df.columns = ['Gesamtfallzahlen']
df['neu'] = infn
print(df)
df.to_csv(f'Germany/NRW/REK/data/Rhein-Erft-Kreis_{tod}.csv')
...
old7 = pd.read_csv(f'Germany/NRW/REK/data/Rhein-Erft-Kreis_{da7s}.csv', index_col = 0)
old14 = pd.read_csv(f'Germany/NRW/REK/data/Rhein-Erft-Kreis_{da14s}.csv', index_col = 0)
...
print(zus)
zus.to_csv(f'Germany/NRW/REK/data/Rhein-Erft-Kreis_for_dw14_7.csv') 
...
try:        
    with open(f'Rhein-Erft-Kreis.html', 'w', encoding="utf-8") as out:
