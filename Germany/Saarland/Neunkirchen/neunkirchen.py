...
for x in range(1,15):
    ...
    for para in htmlParse.find_all("p"): 
        t = para.get_text()
        matches = re.findall(', den '+tod+'.*', t)
        if matches:
            if re.findall('Illingen \+?(\d+)',matches[0]):
                ne = re.findall('Neunkirchen \+?(\d+)',matches[0])[0]
                il = re.findall('Illingen \+?(\d+)',matches[0])[0]
                ot = re.findall('Ottweiler \+?(\d+)',matches[0])[0]
                ep = re.findall('Eppelborn \+?(\d+)',matches[0])[0]
                me = re.findall('Merchweiler \+?(\d+)',matches[0])[0]
                sc = re.findall('Schiffweiler \+?(\d+)',matches[0])[0]
                sp = re.findall('Spiesen-Elversberg \+?(\d+)',matches[0])[0]
