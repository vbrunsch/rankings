...
neu = neu.loc[:, pd.notnull(neu.columns)]
neu = neu.dropna(axis=0, how='all')
neu = neu.drop('15.04.2021', axis = 1)
neu = neu[:-2]
neu['Stadt/Gemeinde'] = neu.index
neu = neu.replace('Weiskirchen Neu','Weiskirchen')
neu = neu.replace('Perl Neu','Perl')
neu = neu.replace('Losheim Neu','Losheim am See')
neu = neu.replace('Beckingen Neu','Beckingen')
neu = neu.replace('Mettlach Neu','Mettlach')
...
zus.to_csv(f'Germany/Saarland/Merzig-Wadern/data/Merzig-Wadern_for_dw14_7.csv')
print(zus) 
...
try:        
    with open(f'Merzig-Wadern.html', 'w', encoding="utf-8") as out:
