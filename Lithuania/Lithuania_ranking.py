...
import pandas as pd
df = pd.read_json('ftp://atviriduomenys.nvsc.lt/COVID19.json')

lasdate = df.sort_values(['Atvejo patvirtinimo data'], ascending=[True])
ld = lasdate['Atvejo patvirtinimo data'].iloc[-1]

cols=['Municipality','COVID-Free Days','New Cases in Last 14 Days', 'Last7', 'Previous7']
collect = []

for d in df['Savivaldybė'].unique():
    if d!= '':
        focus = df[df['Savivaldybė']==d]
        #local transmission only - that is no imported
        focus = focus[focus['Įvežtinis']=='Ne']
        sor = focus.sort_values(['Atvejo patvirtinimo data'], ascending=[True])
        sor['new']=1
        sor = sor.groupby('Atvejo patvirtinimo data').sum()

        sor.index = pd.to_datetime(sor.index)
        idx = pd.date_range('02/28/2020', ld)
        ave = sor.reindex(idx, fill_value=0)
...
try:        
    with open(f'Lithuania.html', 'w', encoding="utf-8") as out:
